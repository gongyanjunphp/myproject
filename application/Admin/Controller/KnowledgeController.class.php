...
class  KnowledgeController extends CommonController{
    function index(){
        ...
        //2.调用select方法查询全部数据
        $knowledge_list = $knowledge->page($pageno,$pagesize) ->select();
        //3.分配查询结果到视图
    ...
    function edit(){
        ...
        $knowledge = D('knowledge');
        //3. 根据id查询部门信息
        $knowledge_info = $knowledge->find($id);
        //4. 分配到模板
        $this->assign('info', $knowledge_info);
        //5. 调用模板显示
        $this->display();
    }
    ...
    function modify(){
        ...
        $data['id'] = $_POST['id'];
        //6. 调用save方法进行数据表修改操作
        if($knowledge->save($data)){
            $this->success('修改成功', U('index'), 3);
        } else {
            $this->error('修改失败', U('edit', 'id='.$info['knowledge_id']), 3);
        }
    }
}