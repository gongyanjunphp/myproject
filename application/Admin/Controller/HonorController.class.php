...
class  HonorController extends CommonController{
    function index(){
        ...
        $honor = D('honor');
        //2.调用select方法查询全部数据
        $honor_list = $honor->where($where)->page($pageno,$pagesize) ->select();
        //3.分配查询结果到视图
    ...
    function edit(){
        ...
        $honor = D('honor');
        //3. 根据id查询部门信息
        $honor_info = $honor->find($id);
        //4. 分配到模板
        $this->assign('info', $honor_info);
        //5. 调用模板显示
        $this->display();
    }
    ...
    function modify(){
        ...
        if($picpath){
            unlink ($honor_info['picpath']);
            $path = $picpath;
        }else{
            $path = $honor_info['picpath'];
        }
