...
class  LiveController extends CommonController{
    function index(){
        ...
        $live = D('live');
        //2.调用select方法查询全部数据
        $live_list = $live->page($pageno,$pagesize) ->select();
        //3.分配查询结果到视图
    ...
    function modify(){
        ...
        if($picpath){
            unlink ($live_info['picpath']);
            $path = $picpath;
        }else{
            $path = $live_info['picpath'];
        }
        ...
        if($live->save($data)){
            ...
            $this->error('修改失败', U('edit', 'id='.$info['live_id']), 3);
        }
    }
